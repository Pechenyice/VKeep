...
function ProfileStatInfo({content}) {
    ...
    return (
        !content.sessions.needFetch && Object.keys(content.userInfo).length && Object.keys(content.updates).length ? 
        <div className={styles.statInfoWrapperContainer}>
            ...
            <StatInfoSpec icon={svgManager.getSvg('statSessions')} fillSkeleton={false} hint={'сессии за сегодня'} count={content.sessions.entities.length}/>
        ...
        <div className={styles.statInfoWrapperContainer}>
            ...
            <StatInfoSpec icon={svgManager.getSvg('statSessions')} fillSkeleton={true} skeleton={<Skeleton type={'text'} animatorBackground={'var(--white)'} stylesheet={{width: '50%', marginTop: '10px'}} />} hint={'сессии за сегодня'} count={content.sessions.entities.length}/>
